...
bibliography: aefp_poster/packages.bib
---
...
California passed its charter school law in 1992 and has seen steadily increased growth in the past two decades.
...
District differ in the share of schools are charter and traditional.
```{r}
dist_info <- 
  charter_dat %>% 
  group_by(leaid) %>% 
  ...
  summarize(sum_threat = sum(change_threat, na.rm = TRUE), 
            ...
            n_charter = sum(n_charter, na.rm = TRUE)) %>% 
  ungroup() %>%
  ...
  mutate(name = case_when(
    name == "increase_threat" ~ "% increased exposure",
    name == "increase_white" ~ "% increased share of white students in charters",
    name == "charters" ~ "% charters"
  ))
...
bind_rows(dist_info, dist_demo) %>% 
  ...
  insert_row(c("", "Mean", "SD"), after = 3) %>% 
  set_bottom_border(c(1, 9), 1:3, 1) %>% 
  set_bottom_border(4, 2:3, 1)
```
...
tab <- huxreg(models1, 
  ...
  insert_row(c("District Fixed Effects", rep("X", 5)), after = 11) %>% 
  insert_row(c("Year Fixed Effects", rep("X", 5)), after = 12) %>% 
  ...
  insert_row(c("Table 1: Fixed effects regression of share of charterst \non proximal and direct threat", rep("", 5)), colspan = 1, after = 0)
...
tab <- huxreg(models1, 
  ...
  set_bottom_border(13, 2:6, 0) %>% 
  set_bottom_border(14, 2:6, 0) %>% 
  set_align(1:13, 2:5, "center") %>% 
  insert_row(c("Table 2: Fixed effects regression of share of white students enrolled in charters \non proximal and direct threat", rep("", 5)), colspan = 6, after = 0)
...
huxreg(models, 
  ...
  insert_row(c("Table 3. Results of 2SLS regression of threat-induced enrollment in charters on white isolation.", rep("", 6)), colspan = 7) %>% 
  set_wrap(2, 2:7, TRUE)
```
